...
int main(int argc, char **argv)
{
	...
	do
	{
		...
		if( seq_len != 0 )
		{
			...
			if ( r )
			{
				...
				#ifdef _USE_64
				fprintf( stderr, "Processing both strands of sequence %s of length %lld\n", ( char * ) seq_id, seq_len );
				#endif
			}
			else
			{
				...
				#ifdef _USE_64
				fprintf( stderr, "Processing 5'->3' strand of sequence %s of length %lld\n", ( char * ) seq_id, seq_len );
				#endif
			}
			seq[ seq_len ] = '\0';
	...
	for ( int i = 0; i < num_seqs; i++ )	
	{
		...
		{			
	    compute_dmaw ( seqs[i], seqs_id[i], sw, seqs[j], seqs_id[j], 1 );	
	    compute_dmaw ( seqs[j], seqs_id[j], sw, seqs[i], seqs_id[i], 2 );
    }
  }
